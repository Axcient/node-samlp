...
const PassportProfileMapper = require("./claims/PassportProfileMapper");
...
function buildSamlResponse(options) {
  ...
  if (options.signResponse) {
    options.signatureNamespacePrefix =
      typeof options.signatureNamespacePrefix === "string"
        ? options.signatureNamespacePrefix
        : "";
    ...
    var sig = new SignedXml(null, {
      signatureAlgorithm:
        constants.ALGORITHMS.SIGNATURE[options.signatureAlgorithm],
    });
...
function nameIdentiferNotFoundErrorMessage(options) {
  ...
  var probes = Array.isArray(options.nameIdentifierProbes)
    ? options.nameIdentifierProbes.join(", ")
    : "";
  ...
  saml20.create(
    {
      ...
      nameIdentifierFormat:
        ni.nameIdentifierFormat || options.nameIdentifierFormat,
...
module.exports.auth = function (options) {
  ...
  return function (req, res, next) {
    ...
    function execute(postUrl, audience, req, res, next) {
      ...
      if (!user) {
        var error = new Error("SAML Unauthorized");
      ...
      getSamlResponse(opts, user, function (err, SAMLResponse) {
        ...
        var response = Buffer.from(SAMLResponse);
    ...
    utils.parseSamlRequest(
      req,
      (req.query || {}).SAMLRequest || (req.body || {}).SAMLRequest,
      "AUTHN_REQUEST",
      opts,
      function (err, samlRequestDom) {
        if (err) return next(err);
        ...
        opts.getPostURL(audience, samlRequestDom, req, function (err, postUrl) {
          ...
          if (!postUrl) {
            var error = new Error("SAML Unauthorized, missing postUrl");
...
module.exports.parseRequest = function (req, options, callback) {
  ...
  utils.parseSamlRequest(req, samlRequest, "AUTHN_REQUEST", options, function (
    err,
    samlRequestDom
  ) {
    if (err) {
    ...
    var assertionConsumerUrl = samlRequestDom.documentElement.getAttribute(
      "AssertionConsumerServiceURL"
    );

    if (assertionConsumerUrl)
      data.assertionConsumerServiceURL = assertionConsumerUrl;

    var destination = samlRequestDom.documentElement.getAttribute(
      "Destination"
    );
    ...
    var requestedAuthnContextClassRefElements = xpath.select(
      constants.ELEMENTS.AUTHN_REQUEST.AUTHN_CONTEXT_CLASS_REF_PATH,
      samlRequestDom
    );

    if (
      requestedAuthnContextClassRefElements &&
      requestedAuthnContextClassRefElements.length === 1
    ) {
      data.requestedAuthnContext = {};

      data.requestedAuthnContext.authnContextClassRef =
        requestedAuthnContextClassRefElements[0].textContent;
    }
