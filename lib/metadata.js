...
const URL_PATH = "/FederationMetadata/2007-06/FederationMetadata.xml";
...
function getEndpointAddress(req, endpointPath) {
  ...
  const protocol =
    req.headers["x-iisnode-https"] && req.headers["x-iisnode-https"] === "on"
      ? "https"
...
function metadataMiddleware(options) {
  ...
  const claimTypes = (options.profileMapper || PassportProfileMapper).prototype
    .metadata;

  const issuer = options.issuer;
  const pem = encoders.removeHeaders(options.cert);

  return function (req, res) {
    const redirectEndpoint = getEndpointAddress(
      req,
      options.redirectEndpointPath
    );

    const postEndpoint = getEndpointAddress(req, options.postEndpointPath);

    options.logoutEndpointPaths = options.logoutEndpointPaths || {
      redirect: "/logout",
    };
    ...
    ["redirect", "post"].forEach(function (binding) {
      if (options.logoutEndpointPaths[binding]) {
        logoutEndpoints[binding] = getEndpointAddress(
          req,
          options.logoutEndpointPaths[binding]
        );
      }
    });
