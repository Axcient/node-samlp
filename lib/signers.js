...
module.exports.signXml = function (options, xml) {
  ...
  sig.addReference(
    options.reference ||
      constants.ELEMENTS.LOGOUT_REQUEST.SIGNATURE_LOCATION_PATH,
    [
...
module.exports.validateXmlEmbeddedSignature = function (xml, options) {
  ...
  if (options.thumprints) {
    ...
    options.thumbprints =
      options.thumbprints instanceof Array
        ? options.thumbprints
        : [options.thumbprints];
  }
  ...
  if (options.thumbprints) {
    var validThumbprint = options.thumbprints.some(function (thumbprint) {
...
module.exports.isValidContentAndSignature = function (
  content,
  signature,
  options
) {
  var verifier = crypto.createVerify(
    options.signatureAlgorithm.split("#")[1].toUpperCase()
  );
