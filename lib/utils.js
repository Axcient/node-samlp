...
module.exports.parseSamlRequest = function (
  req,
  samlRequest,
  type,
  options,
  ...
  function configureSigningCredentials(xml, cb) {
    ...
    if (constants.ELEMENTS[type].SESSION_INDEX_PATH) {
      var sessionIndexNode = xpath.select(
  ...
  try {
    var input = Buffer.from(samlRequest, "base64");
...
function validateSignature(req, element_type, xml, options) {
  ...
  var isRequestSigned = !options.deflate
    ? xpath.select(
        options.signaturePath ||
          constants.ELEMENTS[element_type].SIGNATURE_VALIDATION_PATH,
        xml
      ).length > 0
    : !!req.query.Signature;
  ...
  if (isRequestSigned) {
    if ((req.body && req.body[type]) || !options.deflate) {
      ...
      var valid = signers.isValidContentAndSignature(
