...
describe("samlp", function () {
  ...
  describe("SAMLRequest on querystring", function () {
    ...
    it("should map every attributes from profile", function () {
      function validateAttribute(position, name, value, type, nameFormat) {
        expect(attributes[position].getAttribute("Name")).to.equal(name);
        expect(attributes[position].getAttribute("NameFormat")).to.equal(
          nameFormat
        );
        expect(
          attributes[position].firstChild.getAttribute("xsi:type")
        ).to.equal(type);
        expect(attributes[position].firstChild.textContent).to.equal(value);
      }

      validateAttribute(
        0,
        "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier",
        String(server.fakeUser.id),
        "xs:double",
      ...
      validateAttribute(
        ...
        server.fakeUser.emails[0].value,
        "xs:string",
        "urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
      );
      validateAttribute(
      ...
      validateAttribute(
        ...
        server.fakeUser.name.givenName,
        "xs:string",
        "urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
      );
      validateAttribute(
        ...
        server.fakeUser.name.familyName,
        "xs:string",
        "urn:oasis:names:tc:SAML:2.0:attrname-format:uri"
      );
    });
    ...
    it("should contains the name identifier", function () {
      expect(xmlhelper.getNameIdentifier(signedAssertion).textContent).to.equal(
        String(server.fakeUser.id)
      );
    });
    ...
    it("should set nameidentifier format to urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified by default", function () {
      expect(
        xmlhelper.getNameIdentifier(signedAssertion).getAttribute("Format")
      ).to.equal("urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified");
    });

    it("should contains the issuer", function () {
      expect(xmlhelper.getIssuer(signedAssertion)).to.equal("urn:fixture-test");
    });

    it("should contains the audiences", function () {
      expect(xmlhelper.getAudiences(signedAssertion)[0].textContent).to.equal(
        "https://auth0-dev-ed.my.salesforce.com"
      );
    });
    ...
    it("should use the default authnContextClassRef", function () {
      expect(
        xmlhelper.getAuthnContextClassRef(signedAssertion).textContent
  ...
  describe("when sending SAMLRequest ID ", function () {
    ...
    it("should send back the ID as InResponseTo", function () {
      expect(
        xmlhelper
          .getSubjectConfirmationData(signedAssertion)
    ...
    describe("HTTP Redirect", function () {
      describe("when sending a not signed SAMLRequest", function () {
        ...
        it("return signature missing error", function () {
          expect(error).to.equal(
            "SAMLRequest message MUST be signed when using an asynchronous binding (POST or Redirect)"
          );
        });
      });
      ...
      describe("when sending a signed SAMLRequest with ID that doesn't match", function () {
        ...
        it("should return signature check errors", function () {
          expect(error).to.equal(
            "Signature check errors: invalid signature: the signature refernces an element with uri #pfx41d8ef22-e612-8c50-9960-1b16f15741b3 but could not find such element in the xml"
          );
        });
      });
      ...
      describe("when sending a invalid signed SAMLRequest", function () {
        ...
        it("should return signature check errors", function () {
          expect(error).to.equal(
            "Signature check errors: invalid signature: for uri #pfx41d8ef22-e612-8c50-9960-1b16f15741b3 calculated digest is CNSDTrlQsaLjOFN4js626JZBqP0= but the xml to validate supplies digest yJN6cXUwQxTmMEsPesBP2NkqYFI="
          );
        });
      });
      ...
      describe("when sending a valid signed SAMLRequest but wrong certificate", function () {
        ...
        it("should return invalid signature", function () {
          expect(error).to.equal(
            "Signature check errors: invalid signature: the signature value g5eM9yPnKsmmE/Kh2qS7nfK8HoF6yHrAdNQxh70kh8pRI4KaNbYNOL9sF8F57Yd+jO6iNga8nnbwhbATKGXIZOJJSugXGAMRyZsj/rqngwTJk5KmujbqouR1SLFsbo7Iuwze933EgefBbAE4JRI7V2aD9YgmB3socPqAi2Qf97E= is incorrect"
          );
        });
      });
    ...
    describe("HTTP Redirect - Inflated", function () {
      ...
      describe("when valid algorithm and invalid signature", function () {
        ...
        it("should return missing signature check errors", function () {
          expect(error).to.equal(
            "Signature check errors: The signature provided (123123) does not match the one calculated"
          );
        });
      });
